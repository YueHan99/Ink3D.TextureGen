...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        # (Pdb) inputs['latents'].shape
        # torch.Size([1, 48, 13, 30, 52])  49  480*832
    ...
    def from_pretrained(
        ...
        if isinstance(dit, list):
            ...
            old_conv = dit[0].patch_embedding # Conv3d(52, 5120, kernel_size=(1, 2, 2), stride=(1, 2, 2)) torch.Size([5120, 52, 1, 2, 2])
            new_conv = nn.Conv3d( 
                in_channels=68,
            ...
            # old_weight = old_conv.weight.data  # 不带梯度
            # new_weight = old_weight
            # new_conv.weight.data = new_weight
            # if new_conv.bias is not None:
            #     new_conv.bias.data = old_conv.bias.data.clone()
            pipe.dit.patch_embedding = new_conv
            pipe.dit.patch_embedding = pipe.dit.patch_embedding.to(device=old_conv.weight.device, dtype=old_conv.weight.dtype)

            old_conv2 = dit[1].patch_embedding # Conv3d(52, 5120, kernel_size=(1, 2, 2), stride=(1, 2, 2)) torch.Size([5120, 52, 1, 2, 2])
            new_conv2 = nn.Conv3d( 
                in_channels=68,
            ...
            # if new_conv.bias is not None:
            #     new_conv.bias.data = old_conv.bias.data.clone()
            pipe.dit2.patch_embedding = new_conv2
            pipe.dit2.patch_embedding = pipe.dit2.patch_embedding.to(device=old_conv2.weight.device, dtype=old_conv2.weight.dtype)
        else:
            pipe.dit = dit
            # import pdb;pdb.set_trace()
            old_conv2 = dit.patch_embedding # Conv3d(52, 5120, kernel_size=(1, 2, 2), stride=(1, 2, 2)) torch.Size([5120, 52, 1, 2, 2])
            new_conv2 = nn.Conv3d( 
                in_channels=68,
                out_channels=old_conv2.out_channels,
                kernel_size=old_conv2.kernel_size,
            ...
            # if new_conv.bias is not None:
            #     new_conv.bias.data = old_conv.bias.data.clone()
            pipe.dit.patch_embedding = new_conv2
            pipe.dit.patch_embedding = pipe.dit.patch_embedding.to(device=old_conv2.weight.device, dtype=old_conv2.weight.dtype)
    ...
    def __call__(
        ...
        normal: Optional[list[Image.Image]] = None,
        ...
        inputs_shared = {
            ...
            "control_video": control_video, "normal": normal,  "reference_image": reference_image, 
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if "first_frame_latents" in inputs_shared and progress_id < self.first_frame_replace_steps:
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
        ...
        # import pdb;pdb.set_trace()
        # (Pdb) input_video.shape
        # torch.Size([1, 3, 121, 1024, 1024])
...
class WanVideoUnit_FunControl(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("control_video", 'normal', "num_frames", "height", "width", "tiled", "tile_size", "tile_stride", "clip_feature", "y", "latents"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, control_video, normal, num_frames, height, width, tiled, tile_size, tile_stride, clip_feature, y, latents):
        # import pdb;pdb.set_trace()
        ...
        normal_video = pipe.preprocess_video(normal)
        normal_latents = pipe.vae.encode(normal_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
        normal_latents = normal_latents.to(dtype=pipe.torch_dtype, device=pipe.device)
        # import pdb;pdb.set_trace()
        y_dim = pipe.dit.in_dim-control_latents.shape[1]-latents.shape[1] # 52 - 16 - 16 = y_dim = 20
        if clip_feature is None or y is None:
            clip_feature = torch.zeros((1, 257, 1280), dtype=pipe.torch_dtype, device=pipe.device)
            y = torch.zeros((1, y_dim, (num_frames - 1) // 4 + 1, height//8, width//8), dtype=pipe.torch_dtype, device=pipe.device)
        else:
            y = y[:, -y_dim:]
        y = torch.concat([control_latents, y, normal_latents], dim=1)
        return {"clip_feature": clip_feature, "y": y}
    
# (Pdb) pipe.dit.in_dim
# 52  
# (Pdb) y.shape 
# torch.Size([1, 20, 21, 60, 104])
# (Pdb) control_latents.shape
# torch.Size([1, 16, 21, 60, 104])
# (Pdb) clip_feature.shape
...
class WanVideoUnit_FunCameraControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, camera_control_direction, camera_control_speed, camera_control_origin, latents, input_image, tiled, tile_size, tile_stride):
        ...
        # torch.Size([81, 480, 832, 6])
        control_camera_latents = torch.concat(
            [
                torch.repeat_interleave(control_camera_video[:, :, 0:1], repeats=4, dim=2), # torch.Size([1, 6, 4, 480, 832])
                control_camera_video[:, :, 1:] # torch.Size([1, 6, 80, 480, 832])
            ], dim=2
        ).transpose(1, 2)
        b, f, c, h, w = control_camera_latents.shape # torch.Size([1, 84, 6, 480, 832])
        ...
        input_latents = pipe.vae.encode(input_latents, device=pipe.device) # torch.Size([1, 16, 1, 60, 104])
        y = torch.zeros_like(latents).to(pipe.device)
        y[:, :, :1] = input_latents # torch.Size([1, 16, 21, 60, 104])
        y = y.to(dtype=pipe.torch_dtype, device=pipe.device)

        if y.shape[1] != pipe.dit.in_dim - latents.shape[1]: # 36 - 16
            image = pipe.preprocess_image(input_image.resize((width, height))).to(pipe.device)
            vae_input = torch.concat([image.transpose(0, 1), torch.zeros(3, num_frames-1, height, width).to(image.device)], dim=1)
            y = pipe.vae.encode([vae_input.to(dtype=pipe.torch_dtype, device=pipe.device)], device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)[0]
            y = y.to(dtype=pipe.torch_dtype, device=pipe.device) # torch.Size([16, 21, 60, 104])
            msk = torch.ones(1, num_frames, height//8, width//8, device=pipe.device) # torch.Size([1, 81, 60, 104])
            msk[:, 1:] = 0
            msk = torch.concat([torch.repeat_interleave(msk[:, 0:1], repeats=4, dim=1), msk[:, 1:]], dim=1) # torch.Size([1, 84, 60, 104])
            msk = msk.view(1, msk.shape[1] // 4, 4, height//8, width//8) # torch.Size([1, 21, 4, 60, 104])
            msk = msk.transpose(1, 2)[0] # torch.Size([4, 21, 60, 104])
            y = torch.cat([msk,y])
            y = y.unsqueeze(0)
            y = y.to(dtype=pipe.torch_dtype, device=pipe.device)
        return {"control_camera_latents_input": control_camera_latents_input, "y": y}
# torch.Size([1, 24, 21, 480, 832])
# (Pdb) y.shape
# torch.Size([1, 20, 21, 60, 104])
# (Pdb) msk[:,0].mean()
...
class WanVideoUnit_VACE(PipelineUnit):
    ...
    def process(
        ...
        if vace_video is not None or vace_video_mask is not None or vace_reference_image is not None:
            ...
            vace_video_latents = torch.concat((inactive, reactive), dim=1) # torch.Size([1, 32, 21, 60, 104])
            ...
            if vace_reference_image is None:
                ...
                # (Pdb) vace_reference_latents.shape
                # torch.Size([1, 32, 1, 60, 104])
                # (Pdb) vace_video_latents.shape
                # torch.Size([1, 32, 21, 60, 104])
                vace_video_latents = torch.concat((vace_reference_latents, vace_video_latents), dim=2)
                vace_mask_latents = torch.concat((torch.zeros_like(vace_mask_latents[:, :, :1]), vace_mask_latents), dim=2)
            
            vace_context = torch.concat((vace_video_latents, vace_mask_latents), dim=1)
            # (Pdb) vace_video_latents.shape
            # torch.Size([1, 32=inactive+reactive, 1 ref + 21 vace video, 60, 104])
            # (Pdb) vace_mask_latents.shape  全1
            # torch.Size([1, 64=8*8空间下采样过来的, 22, 60, 104])
            # (Pdb) vace_context.shape
            # torch.Size([1, 96, 22, 60, 104])   
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    if dit.seperated_timestep and fuse_vae_embedding_in_latents:
        ...
        # (Pdb) t.shape
        # torch.Size([1, 5070, 3072])
        # (Pdb) t_mod.shape
        # torch.Size([1, 5070, 6, 3072])
        # (Pdb) timestep
        # tensor([  0.,   0.,   0.,  ..., 572., 572., 572.], device='cuda:0',
        #        dtype=torch.bfloat16)
        # (Pdb) sinusoidal_embedding_1d(dit.freq_dim, timestep).unsqueeze(0).shape
        # torch.Size([1, 5070, 256])
        # (Pdb) dit.time_embedding
        # Sequential(
    ...
    if timestep.shape[0] != context.shape[0]: # 5070 vs 1
    ...
    # (Pdb) x.shape
    # torch.Size([1, 48, 13, 30, 52])
    # torch.Size([1, 5070, 3072])
    # import pdb;pdb.set_trace()
    # Reference image
    if reference_latents is not None:
        if len(reference_latents.shape) == 5: # torch.Size([1, 16, 1, 60, 104])
            reference_latents = reference_latents[:, :, 0]
        reference_latents = dit.ref_conv(reference_latents).flatten(2).transpose(1, 2) # Conv2d(16, 5120, kernel_size=(2, 2), stride=(2, 2))
        # if torch.rand(1).item() < 0.5:
        #     reference_latents = torch.zeros_like(reference_latents)
        x = torch.concat([reference_latents, x], dim=1) # torch.Size([1, 1560 空间展平下采样2倍, 5120]) + torch.Size([1, 32760 = 21*1560, 5120])
        f += 1 # 22
...
#    (Pdb) len(dit.freqs)
#     3
#     (Pdb) dit.freqs[0].shape
#     torch.Size([1024, 22]) 
    # (Pdb) freqs.shape
    # torch.Size([5070, 1, 64])
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks): # len(dit.blocks) 30
    ...
    if reference_latents is not None:
        ...
        f -= 1
    x = dit.unpatchify(x, (f, h, w)) # (13, 15, 26)
    # torch.Size([1, 48, 13, 30, 52])
    return x
