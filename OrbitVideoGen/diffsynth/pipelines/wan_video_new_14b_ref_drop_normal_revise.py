...
class WanVideoPipeline(BasePipeline):
    ...
    def from_pretrained(
        ...
        old_conv = dit.patch_embedding # Conv3d(52, 5120, kernel_size=(1, 2, 2), stride=(1, 2, 2)) torch.Size([5120, 52, 1, 2, 2])
        new_conv = nn.Conv3d( 
            in_channels=68,
        ...
        # 复制权重：只复制前52个输入通道的权重
        with torch.no_grad():
            # new_conv 的权重形状: (out_channels, 68, depth, height, width)
            # old_conv 的权重形状: (out_channels, 52, depth, height, width)
            new_conv.weight[:, :52, :, :, :] = old_conv.weight

            # 如果有偏置，直接复制（因为 out_channels 没变）
        ...
        dit.patch_embedding = dit.patch_embedding.to(device=old_conv.weight.device, dtype=old_conv.weight.dtype)
