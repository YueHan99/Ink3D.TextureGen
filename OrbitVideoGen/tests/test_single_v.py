"""
Single-sample inference: V-track (vertical orbit, 768x768)

Usage:
  export PYTHONPATH="/home/v-hanyue/workspace/Ink3D/OrbitVideoGen:${PYTHONPATH}"
  python tests/test_single_v.py --ref_image /path/to/ref.png --video_dir /path/to/v120/ [--output output_v.mp4]
...
def main():
    parser = argparse.ArgumentParser(description="Single-sample inference (V-track, 768x768)")
    ...
    parser.add_argument("--output", type=str, default="./output_v.mp4", help="Output video path")
    ...
    # Inject patch embedding + LoRA (TODO: swap to V-track ckpt when available)
