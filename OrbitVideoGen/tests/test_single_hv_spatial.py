"""
Single-sample inference: H+V spatial concat (1024x512, 121 frames)

Output format: 4-panel [ref | condition | generated | albedo], 1024 wide per panel
The generated panel is H(512) | V(512) side-by-side.

Usage:
  export PYTHONPATH="/path/to/OrbitVideoGen:${PYTHONPATH}"
  python tests/test_single_hv_spatial.py --ref_image /path/to/ref.png \
      --video_dir /path/to/hv120/ [--output output.mp4]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Inference: H+V spatial concat (1024x512)")
    parser.add_argument("--ref_image", type=str, required=True)
    parser.add_argument("--video_dir", type=str, required=True,
        help="Directory with hv120 condition videos (1024 wide)")
    parser.add_argument("--output", type=str, default="./output_hv_spatial.mp4")
    ...
    # Load data
    def load_frames(video_path, w):
        vd = VideoData(video_path, height=HEIGHT, width=w)
    ...
    condition_video_frames = load_frames(os.path.join(args.video_dir, "position.mp4"), WIDTH)
    normal_video_frames = load_frames(os.path.join(args.video_dir, "normal.mp4"), WIDTH)
    albedo_video_frames = load_frames(os.path.join(args.video_dir, "albedo.mp4"), WIDTH)
    ...
    if not args.no_bg_remove:
        ...
        reference_image = remove_background(reference_image.convert("RGBA"))
    ...
    generated_video = pipe(
        ...
        num_frames=min_len, height=HEIGHT, width=WIDTH,
        seed=args.seed, tiled=True, cfg_scale=args.cfg_scale,
    )
