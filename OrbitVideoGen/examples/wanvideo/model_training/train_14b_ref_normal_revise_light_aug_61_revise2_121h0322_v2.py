...
from diffsynth.trainers.utils_ref_normal_albedo_light_aug_61_revise_imageref_h0322_v2 import VideoDataset
...
class WanTrainingModule(DiffusionTrainingModule):
    def __init__(
        ...
        if lora_base_model is not None or lora_checkpoint is not None:
            ...
            if lora_checkpoint is not None:
                ...
                print(f"正在加载DIT patch embed模型权重: {lora_checkpoint}")
                # import pdb;pdb.set_trace()
            else:
                patch_embedding_state_dict1 = None
        
            setattr(self.pipe, lora_base_model, model)
            if patch_embedding_state_dict1 is not None:
        ...
        print("==================The following modules are trainable===================:")
        ...
        for name, module in self.pipe.named_modules():
            # 检查这个 module 是否有任何参数是 requires_grad == True
    ...
    def forward(self, data, inputs=None):
        # (Pdb) data['video'][0]
        # <PIL.Image.Image image mode=RGB size=832x480 at 0x7ECDEC0D8130>
        # (Pdb) len(data['video'])
        # 49
        # (Pdb) data['prompt']
        ...
        # (Pdb) self.pipe.in_iteration_models
        # ('dit', 'motion_controller', 'vace')
