...
Usage:
  python batch_render_mr.py --input_json glb_list.json \
...
def render_orbit(glb_path, relative_path, output_dir, orbit, args):
    ...
    try:
        ...
        if r.returncode != 0:
            with print_lock:
                print(f"\n[ERROR] MR {orbit} render failed for {relative_path}")
            return False
        return True
    except subprocess.TimeoutExpired:
        with print_lock:
            print(f"\n[ERROR] MR {orbit} render timeout for {relative_path}")
        return False
...
def main():
    parser = argparse.ArgumentParser(description="Batch MR rendering")
