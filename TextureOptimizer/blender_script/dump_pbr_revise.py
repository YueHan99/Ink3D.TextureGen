...
def main(arg):    
    ...
    for mat_idx, mat in enumerate(bpy.data.materials):
        pack = {
            "baseColorFactor": [1.0, 1.0, 1.0, 1.0],  # RGBA
            "alphaFactor": 1.0,
            "metallicFactor": 0.0,     # 默认非金属
            "roughnessFactor": 0.5,    # 默认中等粗糙度
            "alphaMode": "OPAQUE",
            "alphaCutoff": 0.5,
        ...
        if not mat.use_nodes:
            print(f"[WARN] Material '{mat.name}' has no node tree, using default PBR params.")
            # 尝试从 mat.diffuse_color 获取基础颜色（Blender 旧版材质）
            if hasattr(mat, 'diffuse_color'):
                pack["baseColorFactor"] = list(mat.diffuse_color) + [1.0]
            if hasattr(mat, 'alpha'):
                pack["alphaFactor"] = mat.alpha
                if pack["alphaFactor"] < 1.0:
                    pack["alphaMode"] = "BLEND"
            output['materials'].append(pack)
            continue

        # 情况2: 有节点树，尝试找 Principled BSDF
        principled_node = mat.node_tree.nodes.get('Principled BSDF')
        
        if principled_node is None:
            # 降级方案：尝试找 Diffuse BSDF 或 Emission 作为备选
            fallback_node = None
            for node in mat.node_tree.nodes:
                if node.type in ['BSDF_DIFFUSE', 'BSDF_PRINCIPLED', 'EMISSION', 'RGB']:
                    fallback_node = node
                    break
            
            if fallback_node:
                print(f"[WARN] Material '{mat.name}' has no Principled BSDF, using fallback node '{fallback_node.name}'.")
                if fallback_node.type == 'BSDF_DIFFUSE':
                    if not fallback_node.inputs['Color'].is_linked:
                        pack["baseColorFactor"] = list(fallback_node.inputs['Color'].default_value) + [1.0]
                elif fallback_node.type == 'EMISSION':
                    if not fallback_node.inputs['Color'].is_linked:
                        pack["baseColorFactor"] = list(fallback_node.inputs['Color'].default_value) + [1.0]
                        pack["roughnessFactor"] = 0.0  # Emission 通常光滑
                elif fallback_node.type == 'RGB':
                    pack["baseColorFactor"] = list(fallback_node.outputs[0].default_value) + [1.0]
            else:
                print(f"[WARN] Material '{mat.name}' not supported, using default PBR params.")
            output['materials'].append(pack)
            continue

        # 情况3: 正常处理 Principled BSDF（原逻辑）
        try:
            # Handle base color
            if not principled_node.inputs['Base Color'].is_linked:
                pack["baseColorFactor"] = list(principled_node.inputs['Base Color'].default_value) + [1.0]
            else:
                link = principled_node.inputs['Base Color'].links[0]
                if link.from_node.type == 'RGB':
                    pack["baseColorFactor"] = list(link.from_node.outputs[0].default_value) + [1.0]
                else:
                    factor, image = try_extract_image_with_factor(link, 'RGB')
                    # factor 是 RGB 三元组，需要补全 A=1.0
                    pack["baseColorFactor"] = factor + [1.0] if len(factor) == 3 else factor
            ...
            # 捕获异常但不崩溃，记录错误并继续使用默认参数
            error_log = f"[ERROR] Material '{mat.name}' parsing failed: {str(e)}"
            print(error_log)
            with open(arg.output_path + '_material_errors.txt', 'a') as f:
                f.write(error_log + '\n')
            output['materials'].append(pack)  # 使用默认参数
